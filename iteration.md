...
### 2026-08-30 16:17 | Synchronize the website with the latest CV
- Goal: Update the public CV and related project pages from the latest approved CV while preserving previously fact-checked contribution boundaries and adding the undergraduate KV-cache thesis to Selected Work.
- Changed files: `_pages/cv.md`, `_portfolio/2026-05-15-kv-cache-quantization.html`, `_portfolio/2024-09-10-smart-home-terminal.html`, `_portfolio/2025-07-15-deformation-reconstruction.html`, `_portfolio/2025-01-30-climbing-hand-exo.html`, `_includes/seo.html`, `sitemap.xml`, `scripts/check_site_content.rb`, and `iteration.md`.
- Commands: latest Word CV and final-thesis comparison; two independent read-only content reviews; `ruby -c scripts/check_site_content.rb`; repeated `make check`; `xmllint --noout _site/sitemap.xml`; task-scoped and repository-wide `git diff --check`; local Jekyll preview; 1280 x 900 and 390 x 844 in-app browser inspection across Home, CV, Portfolio, KV Cache, and Smart Home; desktop and mobile Gallery open/close tests.
- Outputs: Added explicit Contact labels, corrected the Penn degree name, removed UCLA Math 156 and the Huashu Cup line, expanded the Notre Dame program name, and synchronized Smart Home to Sep 2024 – Jan 2025. Rewrote the KV-cache CV entry in overview, technical, and results order; added the two controlled Needle pass rates, the 73.4% KV-cache storage result, and the conditional 190.23 ms to 113.16 ms within-INT4 TPOT result with model, hardware, and protocol limits. Selected Work now presents KV Cache, Deformation Reconstruction, and the Ball Robot in that order, while the Exoskeleton remains in the complete Portfolio. Portfolio sort dates no longer leak as article publication dates or sitemap last-modified dates.
- Validation: Jekyll 3.9.5 built successfully and `Site content checks passed.` after the final content and sitemap assertions; Ruby syntax and XML parsing passed; the task-scoped diff check passed. Desktop and mobile pages showed no horizontal overflow, missing visible images, console errors, or warnings. Portfolio covers retained equal 4:3 rendering, the three homepage cards retained equal image dimensions, and KV Gallery images opened at their source resolution with scrolling locked and restored correctly. The repository-wide diff check still reports only the pre-existing `format_images.py:102` trailing blank line, which remains outside this task.
- Risks / follow-ups: The 100% figures remain scoped to two controlled Needle tests; 73.4% is KV-cache storage rather than total GPU memory; and the approximately 40% TPOT reduction is relative to the reference INT4 path, not FP16. The exact Smart Home sorting day is deliberately suppressed because only the ending month is confirmed. The unrelated `format_images.py` modification remains unstaged and unchanged.
- Commit: This entry is included in the latest-CV website synchronization commit.
